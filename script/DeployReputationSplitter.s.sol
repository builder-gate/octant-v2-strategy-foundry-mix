 ...
 * @notice Script to deploy ReputationSplitter contract (uses native ETH)
 * @dev Run with: forge script script/DeployReputationSplitter.s.sol --rpc-url $RPC_URL --broadcast --private-key $PRIVATE_KEY
 *
 * Steps:
 * 1. Deploy ReputationSplitter (no token address needed, uses ETH)
 */
contract DeployReputationSplitter is Script {
    ...
    function run() external returns (DeploymentResult memory) {
        ...
        console2.log("Reward Type: Native ETH");
        console2.log("=================================================");
        ...
        ReputationSplitter splitter = new ReputationSplitter();
